...
<?php
/***** gallery settings *****/
$mainFolder          = '_albums'; // main folder that holds albums - this folder resides on root directory of your domain
...
$itemsPerPage        = '16';    // number of images per page if not already specified in ajax mode
...
$mainVideo          = '_videos'; // main folder that holds albums - this folder resides on root directory of your domain
$extensionsVideo    = array("avi","mp4","wmv"); // allowed extensions in video gallery
$num_video_captions_chars  = '30';    // number of characters displayed in album and thumb captions
...
{		
    $albums = array_diff(scandir($mainFolder), array('..', '.', '.DS_Store', '.htaccess'));
    ...
    $videos = array_diff(scandir($mainVideo), array('..', '.', '.DS_Store', '.htaccess'));
    ...
    if ($numVideos <= 0) :
        ...
        <div class="titlebar"><p>There are currently no videos</p></div>

        <?php

    else :
        sort_array($videos,$mainVideo,$sort_albums_by_date); // rearrange array either by date or name

        ?>

        <br><br><br>
        <div class="p10-lr">
            <span class="title">Video Gallery</span> - <?php echo $numVideos; ?> videos
        </div>
        <div class="clear"></div>

        <?php
        for( $i=0; $i<$numVideos; $i++ ) :
            if( !isset($videos[$i])) continue;
            ?>

            <div class="thumb-wrapper">
                <div class="thumb">
                    <a class='flowplayer' rel="<?= $videos[$i]; ?>" href="foliogallery/show_video_html5.php?video=<?php echo $mainVideo.'/'.$videos[$i]; ?>&innerWith=<?php echo $videoFrameWidth; ?>&innerHeight=<?php echo $videoFrameHeight; ?>">
                        <img alt="<?php echo $numVideos[$i]; ?>" src="foliogallery/video_thumb.png">
                    </a>
                </div>
                <div class="caption"><?php echo substr($videos[$i],0,$num_video_captions_chars); ?></div>
            </div>
...
else //display photos in album 
{
	...
	if (!in_array($requested_album, $albums_in_maindir)) { ?>
        <span class="title">Photo Gallery &raquo; <a href="<?php echo $album_page_url; ?>" class="refresh">Root</a></span>
		<p></p>
	...
	$files = array_diff($scanned_album, array('..', '.','thumbs','descriptions.txt','.DS_Store','.htaccess'));
	$numFiles = count($files); ?>
	 
	<div class="p10-lr">
		<?php if($fullAlbum==1) { ?>
			<span class="title"><a href="<?php echo $album_page_url; ?>" class="refresh">Root</a></span>
